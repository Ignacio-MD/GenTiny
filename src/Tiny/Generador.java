...
public class Generador {
    ...
    private static void generar(NodoBase nodo){
        if(tablaSimbolos!=null){
            if (nodo instanceof  NodoIf){
	    		...
	    		generarLeer(nodo);
	    	}else if (nodo instanceof  NodoDecrementar){  /* NUEVO: Decrementar */
	    		generarDecrementar(nodo);
	    	}else if (nodo instanceof  NodoIncrementar){ /* NUEVO: Incrementar */
	    		generarIncrementar(nodo);
    ...
    /* Metodo para INCREMENTAR una variable */
    private static void generarIncrementar(NodoBase nodo){
	    NodoIncrementar n = (NodoIncrementar)nodo;
		int direccion;
		if(UtGen.debug)	UtGen.emitirComentario("-> Incrementar");
                
        direccion = tablaSimbolos.getDireccion(n.getIdentificador());
        UtGen.emitirRM("LD", UtGen.AC, direccion, UtGen.GP, "cargo el valor a incrementar de la memoria a r0");
		UtGen.emitirRM("LDC", UtGen.AC1, 1, 0, "cargo el valor 1 a r");
        UtGen.emitirRO("ADD", UtGen.AC, UtGen.AC1, UtGen.AC, "Sumo 1");
		UtGen.emitirRM("ST", UtGen.AC, direccion, UtGen.GP, "INCREMENTAR: se incremento al valor de id "+n.getIdentificador());
		
        if(UtGen.debug)	UtGen.emitirComentario("<- Incrementar");
	}
        
    /* Metodo para DECREMENTAR una variable*/
    private static void generarDecrementar(NodoBase nodo){
        NodoDecrementar n = (NodoDecrementar)nodo;
		int direccion;
		if(UtGen.debug)	UtGen.emitirComentario("-> Decrementar");
                
        direccion = tablaSimbolos.getDireccion(n.getIdentificador());
        UtGen.emitirRM("LD", UtGen.AC, direccion, UtGen.GP, "cargo el valor a decrementar de la memoria a r0");
		UtGen.emitirRM("LDC", UtGen.AC1, 1, 0, "cargo el valor 1 a r");
        UtGen.emitirRO("SUB", UtGen.AC, UtGen.AC, UtGen.AC1, "SE RESTA 1");
		UtGen.emitirRM("ST", UtGen.AC, direccion, UtGen.GP, "DECREMENTAR: se decremento al valor de id "+n.getIdentificador());
